...
Example:
    nauta_client = NautaClient("yunior@nauta.com.cu", "yunior")
    try:
...
class NautaProtocol(object):
    ...
    def create_session(cls, default_check_page=CHECK_PAGE):
        ...
        resp = session.requests_session.get(default_check_page)
        if not resp.ok:
...
class NautaClient(object):
    def __init__(self, user, password, default_check_page):
        self.user = user
        self.password = password
        self.session = None
        self.default_check_page = default_check_page

    def init_session(self):
        self.session = NautaProtocol.create_session(default_check_page=self.default_check_page)
        self.session.save()
